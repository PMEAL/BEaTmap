...
class Testio (unittest.TestCase):
    ...
    def setup_class(self):
        ...
        self.bad_list_shape_test = {'relp': [[.1, .2, .21, .3, .4, .5], [.1, .2, .21, .3, .4, .5]],
                                    'n': [.001, .002, .004, .005, .0055, .006], 
                                    'file': 'my file',
                                    'info': 'my info',
                                    'a_o': 11.11}
    ...
    def test_import_data(self):
        ...
        temp = bt.io.import_data(**self.n_zero_test)
        assert temp.iso_df.equals(bt.io.import_data(**self.n_zero_test).iso_df)
        assert temp.file == self.n_zero_test['file']
        assert temp.info == self.n_zero_test['info']
        assert temp.a_o == self.n_zero_test['a_o']
